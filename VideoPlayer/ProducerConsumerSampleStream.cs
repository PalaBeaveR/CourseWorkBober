...
public class ProducerConsumerSampleStream : Stream {
  ...
  public override int Read(byte[] buffer, int offset, int count) {
    if (count < sizeof(float)) return 0;

    Buffer.BlockCopy(this._buffer, (int)(this._readPosition * sizeof(float)), buffer, offset, sizeof(float)*1024);

    this._readPosition = (this._readPosition + 1024) % this._buffer.Length;
...
//  }
  ...
  public void WriteFloats(float[] buffer) {
    ...
    for (int i = 0; i < buffer.Length; i++) {
      pos = (int)((this._writePosition + i) % this._buffer.Length);
      this._buffer[pos]   = buffer[i];
    }
