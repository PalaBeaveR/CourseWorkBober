...
namespace App {
    ...
    {
        [PlantUmlAssociation(Name = "FFMVideo", Association = "o-->", LeafLabel = "_video")]
        private FFMVideo? _video;

        [PlantUmlAssociation(Name = "OpenGLDisplay", Association = "o-->", LeafLabel = "frame")]
        private int _nothing;

        [PlantUmlIgnoreAssociation]
        public FFMVideo? Video => this._video;
        ...
        private void timeline_OnScroll(object sender, EventArgs e)
        {
            ...
            this._queuedFrame
              = this.timeline.Value / (double)this.timeline.Maximum * this._video.Duration;
            if (!this._timer.IsRunning) this.RenderNextFrame(TimeSpan.FromSeconds(10));
        }

        private void RenderNextFrame(TimeSpan delta)
        {
            this.fpsLabel.Text = ((int)(TimeSpan.FromSeconds(1) /
                                      delta)).ToString(CultureInfo.CurrentCulture);
